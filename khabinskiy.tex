...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[RE,RO]{J S\&E \textbf{2}, 32--34 (2026)}
\fancyhead[LO]{Khabinskiy \emph{et al}}
\fancyhead[LE]{Investigation of free fall}
\fancyfoot[C]{\thepage}
\fancypagestyle{mytitlepage}{
\fancyhf{}
\fancyhead[C]{Journal of Science \& Engineering \textbf{2}, 32--34 (2026)}
\fancyfoot[C]{\thepage}
}
...
Galileo's claim of objects falling with the same constant acceleration independent of their mass is presented in his \emph{Discorses on Two New Sciences} \cite{galilei:1638:discorsi, machamer:2021:galileo}. This directly contradicted the Aristotelian view that a heavy body falls faster than a lighter one \cite{aristotle:physics}. In Galilean free fall, acceleration is constant and uniform, and the time required for an object to fall from rest depends only on the height and gravitational acceleration, not on the object's mass. 

The purpose of this experiment was to test Galileo's prediction by dropping two objects of identical shape and volume but different mass from the same height and comparing their fall times. Under Galilean assumptions, the two objects should reach the ground at the same time when dropped from the same height \cite{tipler, openstax, barrons}, demonstrating the same acceleration for objects of different masses but the same shape. The null hypothesis ($H_0$) states that the accelerations are equal, consistent with Galileo's theory that all objects fall with the same acceleration when resistance is neglected. Alternatively, the acclerations could differ, indicating a deviation from Galilean free fall. 
\begin{align}
...
\subsection{Setup}
This experiment consisted of dropping two disposable 8-inch by 2.5-inch plastic water bottles (Natural Spring Water; Trader Joe's; Freehold, NJ) out of a window \qty{5}{\meter} from the ground. One water bottle was filled with fresh tap water. The other water bottle was filled with small rocks (about \qty{0.5}{in} diameter) from a suburban home garden in Manalapan, NJ. The rocks were Vigoro Bagged Marble Chip Landscape Rock (Home Depot; Marlboro, NJ). Bottles were filled with their corresponding contents to the same level (right above the label). The mass of the water bottle filled with water was \qty{0.402}{\kilo\gram}. The mass of the water bottle filled with rocks was \qty{0.605}{\kilo\gram}.
\begin{figure}
\begin{center}
...
For the real-time dropping, two people were stationed on the second floor of Manalapan High School (\ang{40.2896}N, \ang{74.3363}W) to drop the bottles. The rest of the team went to ground level to record the times it took for each bottle to fall. Walkie-talkies (UV-5R; Baofeng; Quanzhou City, China) were used to communicate. We had two digital stopwatches (Pulivia YS-802; Shenzhen, China) to time the fall, as well as a phone to back up the stopwatches. The timers were started when a countdown was announced from the drop team and were stopped when the ground observed the bottles landing. These values were then recorded. We used a smartphone (Samsung A21; Suwon-Si, South Korea) recording in HD at \qty{60}{fps} at 1.0x zoom, placed where the ground team was standing. The video ended up being too large to upload, so we used a classmate’s recording from an iPhone 11 (Apple, Inc; Cupertino, CA) at 60fps and HD. This process was repeated five times for each bottle to account for the confounding variables of the experiment, such as wind, delay from the walkie-talkies, and human error regarding starting and stopping the stopwatches. Damage to the water bottles over repeated drops was fixed by pressing the shape back into its original form. 

The acceleration for each drop was calculated assuming constant, uniform acceleration:
\begin{equation}
a = \frac{2 h}{t^2}
\end{equation}
where $h=\qty{5.0}{\meter}$ is the drop height and $t$ is the measured drop time. The assumption of uniform constant acceleration was later confirmed by digitizing video, as discussed below (please see \cref{fig:2}). Statistical analyses of the drop times and their corresponding acceleration was performed in R \cite{starnes:2015:practice, R}.



\subsection{Analysis}
Videos were analyzed in the app FizziQ (\url{https://www.fizziq.org/en/fizziqclassique}) to obtain video kinematics \cite{chazot:2025:using}. As a second check of our acceleration estimates, acceleration was a;sp found from the velocity data obtained from FizziQ, which was subject to linear regression to determine the slope. Subsequent statistical analysis was performed in R \cite{R} using the \texttt{ggplot2} and \texttt{dplyr} packages \cite{ggplot2,dplyr}. All data and analysis code are available at \url{https://github.com/devangel77b/427jkhabinskiy-lab1}. 




\section{Results}
Drop times and accelerations are summarized in \cref{tab:1}. Differences between rocks and water are not significant for times (ANOVA, $p=0.893$) nor for acceleration (ANOVA, $p=0.963$).
\begin{table}[b]
\caption{Drop times and accelerations (mean $\pm$ 1 s.d.) for $n=5$ drops. Differences between rocks and water are not significant for times (ANOVA, $p=0.893$) nor for acceleration (ANOVA, $p=0.963$).}
\label{tab:1}
\begin{ruledtabular}
\begin{tabular}{lcc}
...
Velocity estimates from digitized data are shown in \cref{fig:2}. Differences between rocks and water are not significant for times (ANOVA, $p=0.893$) nor for acceleration (nested ANOVA, $p=0.119$).
\begin{figure}
\begin{center}
\includesvg{fig2.svg}
\end{center}
